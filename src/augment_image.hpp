...
inline std::ostream& operator << (std::ostream& out, const emit_type& et)
{
    switch (et) {
    case emit_type::center:
        out << "center";
        break;
    case emit_type::min_overlap:
        out << "min_overlap";
        break;
    case emit_type::undefined:
        out << "undefined";
        break;
    }
    return out;
    ...
    namespace image {
      ...
      public:
        explicit sample(float scale, float aspect_ratio)
          : m_scale(scale), m_aspect_ratio(aspect_ratio) {
        }
      ...
      class sampler final : public json_configurable {
      public:
        sampler() = default;
        explicit sampler(const nlohmann::json& config);
        sampler& operator =(const nlohmann::json& config);
      ...
      class sample_constraint final : public json_configurable {
      public:
        sample_constraint() = default;
        explicit sample_constraint(const nlohmann::json& config);
        sample_constraint& operator =(const nlohmann::json& config);
      ...
      class batch_sampler : public json_configurable {
      public:
        batch_sampler() = default;
        explicit batch_sampler(const nlohmann::json& config);
      ...
      private:
        ...
        bool has_max_sample() const {
          return m_max_sample != -1;
        }
        std::vector<std::shared_ptr<interface::config_info_interface>> config_list = {
          ADD_SCALAR_WITH_KEY(
            m_max_sample, "max_sample", mode::OPTIONAL, [](int x) {
            return x >= 0;
          }),
      ...
      class param_factory : public json_configurable {
      public:
        explicit param_factory(nlohmann::json config);
      ...
      private:
        ...
        std::vector<std::shared_ptr<interface::config_info_interface>> config_list = {
          ...
          ADD_DISTRIBUTION(angle, mode::OPTIONAL, [](decltype(angle) v) {
            return v.a() <= v.b();
          }),
          ADD_DISTRIBUTION(lighting, mode::OPTIONAL),
          ADD_DISTRIBUTION(horizontal_distortion,
                           mode::OPTIONAL,
                           [](decltype(horizontal_distortion) v) {
                             return v.a() <= v.b();
                           }),
          ...
          ADD_DISTRIBUTION(
            contrast, mode::OPTIONAL, [](decltype(contrast) v) {
            return v.a() <= v.b();
          }),
          ADD_DISTRIBUTION(
            brightness, mode::OPTIONAL, [](decltype(brightness) v) {
            return v.a() <= v.b();
          }),
          ADD_DISTRIBUTION(
            saturation, mode::OPTIONAL, [](decltype(saturation) v) {
            return v.a() <= v.b();
          }),
          ADD_DISTRIBUTION(expand_ratio,
                           mode::OPTIONAL,
                           [](decltype(expand_ratio) v) {
                             return v.a() >= 1 && v.a() <= v.b();
                           }),
          ADD_DISTRIBUTION(hue, mode::OPTIONAL, [](decltype(hue) v) {
            return v.a() <= v.b();
          }),
#ifdef PYTHON_PLUGIN
