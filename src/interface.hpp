...
namespace nervana {
  ...
  namespace interface {
    ...
    public:
      virtual ~config_info_interface() = default;
...
#define ADD_JSON(var, key, mode, ...)                                                              \
  ...
  namespace interface {
    ...
    class config : public json_configurable {
    public:
      config() = default;
      const shape_type& get_shape_type(size_t index = 0) const {
      ...
      const std::vector<shape_type>& get_shape_type_list() const {
        return m_shape_type_list;
      }

      void add_shape_type(const std::vector<size_t>& shape_, const std::string& output_type_) {
        m_shape_type_list.emplace_back(shape_, output_type{ output_type_ });
      }

      void add_shape_type(const std::vector<size_t>& shape_, const output_type& output_type_) {
        m_shape_type_list.emplace_back(shape_, output_type_);
      }

      void add_shape_type(const std::vector<size_t>& shape_,
                          const std::vector<std::string>& names_,
                          const output_type& output_type_) {
        m_shape_type_list.emplace_back(shape_, output_type_);
        m_shape_type_list.back().set_names(names_);
      }

    private:
      std::vector<shape_type> m_shape_type_list;
    };
    ...
    std::string type_name() {
      ...
      using NonReferenceType = typename std::remove_reference<Type>::type;
    ...
    template<typename T>
    class config_info : public config_info_interface {
    public:
      config_info(
        T& var,
        std::string name,
        config::mode m,
        std::function<void(
          T&, const std::string&, const nlohmann::json&, config::mode)> parse,
        std::function<bool(T)> validate = [](T) -> bool {
          return true;
        })
        : m_target_variable{ var }, m_variable_name{ std::move(name) }, m_parse_mode{ m }, m_parse_function{ parse },
          m_validate_function{ validate }, m_default_value{ var } {
      }

      config_info() = delete;
      ~config_info() override = default;

      const std::string& name() const override {
        return m_variable_name;
      }
      bool required() const override {
        return m_parse_mode == config::mode::REQUIRED;
      }
      std::string type() const override {
        return type_name<T>();
      }
      std::string get_default_value() const override {
        return dump_default(m_default_value);
      }
