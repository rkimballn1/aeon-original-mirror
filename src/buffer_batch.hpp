...
namespace nervana {
  ...
  using variable_record_field = std::vector<char>;
  using variable_record_field_list = std::vector<variable_record_field>;
  ...
  public:
    ...
    std::vector<encoded_record>::const_iterator begin() const {
      return m_records.begin();
    }
    std::vector<encoded_record>::const_iterator end() const {
      return m_records.end();
    }
  ...
  private:
    ...
    std::size_t m_elements_per_record{ std::numeric_limits<std::size_t>::max() };
  };

  class buffer_fixed_size_elements {
  public:
    buffer_fixed_size_elements() = default;
    ...
    const shape_type& get_shape_type() const {
      return m_shape_type;
    }
  ...
  public:
    ...
    fixed_buffer_map() = default;

    fixed_buffer_map(fixed_buffer_map&& buffer) noexcept = default;
    fixed_buffer_map& operator =(fixed_buffer_map&& buffer) noexcept = default;

    void add_items(const std::vector<std::pair<std::string, shape_type>>& write_sizes, size_t batch_size, bool pinned = false) {
    ...
    void add_item(const std::string& name, const shape_type& shp_tp, size_t batch_size, bool pinned = false) {
      ...
      m_data.emplace_back(std::make_pair(name, new buffer_fixed_size_elements(shp_tp, batch_size, pinned)));
    }
