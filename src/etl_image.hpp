...
namespace nervana {
  namespace image {
    ...
    class config : public interface::config {
      friend class nervana::video::config;
      friend class nervana::multicrop::config;
    ...
    private:
      std::vector<std::shared_ptr<interface::config_info_interface>> config_list = {
        ...
        ADD_SCALAR(channels, mode::OPTIONAL, [](uint32_t v) {
          return v == 1 || v == 3;
        }),
...
// ===============================================================================================
    ...
    class decoded : public interface::decoded_image {
    public:
      decoded() {
      }
      decoded(cv::Mat img) {
      ...
      virtual ~decoded() override = default;
    ...
    class extractor : public interface::extractor<image::decoded> {
    public:
      explicit extractor(const image::config&);
      std::shared_ptr<image::decoded> extract(const void*, size_t) const override;

      int get_channel_count() {
          return _color_mode == CV_LOAD_IMAGE_COLOR ? 3 : 1;
      }
    private:
      int _pixel_type;
      int _color_mode;
    ...
    class transformer : public interface::transformer<image::decoded, augment::image::params> {
    public:
      explicit transformer(const image::config&);
    ...
    class loader : public interface::loader<image::decoded> {
    public:
