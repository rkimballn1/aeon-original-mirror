...
{
public:
    thread_barrier(unsigned int count):m_count(count), m_active_thread_count(m_count){}
    void wait()
    {
...
private:
    ...
    void process(int thread_id)
    {
        ...
        for(;;)
        {
            ...
            if (m_thread_pool_stop.load(std::memory_order_relaxed)) return;
            ...
            try
            {
                ...
                else
                {
                    for(;;)
                    {
                        const size_t next_task_id = m_current_task_id.fetch_add(1, std::memory_order_relaxed);
