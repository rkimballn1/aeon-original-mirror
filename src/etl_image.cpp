...
namespace nervana {
  namespace image {
    ...
    config::config(const nlohmann::json& js) {
        if (js.is_null()) {
 ...
 * flip
 */
    cv::Mat transformer::transform_single_image(std::shared_ptr<augment::image::params> img_xform,
    ...
    loader::loader(const image::config& cfg, bool fixed_aspect_ratio)
      : m_channel_major{ cfg.channel_major }, m_fixed_aspect_ratio{ fixed_aspect_ratio },
        m_stype{ cfg.get_shape_type() }, m_channels{ cfg.channels } {
    }

    void loader::load(const std::vector<void*>& outlist, std::shared_ptr <image::decoded> input) const {
        auto outbuf = (char*)outlist[0];
        ...
        std::size_t image_size = img.channels() * img.total() * element_size;
