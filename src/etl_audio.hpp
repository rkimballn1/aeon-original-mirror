  ...
  namespace audio {
    ...
    public:
      ...
      * (e.g. .025 seconds). */
      ...
      uint32_t freq_steps{ 0 };
      uint32_t max_duration_tn{ 0 };
      uint32_t frame_length_tn{ 0 };
      uint32_t frame_stride_tn{ 0 };
      float max_duration_ms{ 0.0f };
      float frame_length_ms{ 0.0f };
      float frame_stride_ms{ 0.0f };
    ...
    private:
      ...
      std::vector<std::shared_ptr<interface::config_info_interface>> config_list = {
        ...
        ADD_DISTRIBUTION(time_scale_fraction,
                         mode::OPTIONAL,
                         [](decltype(time_scale_fraction) v) {
                           return v.a() <= v.b();
                         }),
        // ADD_DISTRIBUTION(noise_index, mode::OPTIONAL),
        ADD_DISTRIBUTION(
          noise_level, mode::OPTIONAL, [](decltype(noise_level) v) {
          return v.a() <= v.b();
        }),
    ...
    class extractor : public interface::extractor<audio::decoded> {
    public:
      extractor() = default;
      std::shared_ptr<audio::decoded> extract(const void*, size_t) const override;
    };

    class transformer : public interface::transformer<audio::decoded, augment::audio::params> {
    public:
      transformer() = delete;
    ...
    class loader : public interface::loader<audio::decoded> {
    public:
      explicit loader(const audio::config& cfg)
        : _cfg{ cfg } {
      }
